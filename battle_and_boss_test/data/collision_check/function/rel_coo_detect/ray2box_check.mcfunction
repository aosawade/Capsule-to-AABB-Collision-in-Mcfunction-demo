#本函数用于相对坐标下的检测定长射线ray是否与轴对齐长方体box相交
...
#范围之外其二，x0>xmin=0
execute unless score #ray posX_0 matches 0.. if score #ray vec_dx matches 0 run return fail
#范围之内，将结果累加在#in_box中
execute if score #ray posX_0 <= #box posX_max if score #ray posX_0 matches 0.. \
...
execute unless score #ray posY_0 matches 0.. if score #ray vec_dy matches 0 run return fail
execute if score #ray posY_0 <= #box posY_max if score #ray posY_0 matches 0.. \
        run scoreboard players add #in_box int 1
...
execute unless score #ray posZ_0 matches 0.. if score #ray vec_dz matches 0 run return fail
execute if score #ray posZ_0 <= #box posZ_max if score #ray posZ_0 matches 0.. \
        run scoreboard players add #in_box int 1
...
#计算x方向上的进出点
#计算(Xmin-X0)/dx
scoreboard players set #temp_en int 0
